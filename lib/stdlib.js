...
textureRFloat32.include = ['textureSize'];
textureRFloat32.type = function (node) {
...
function textureRgbaUnsignedByte(sampler, coord, bias) {
	var size = textureSize(sampler)
	var x = ((coord[0] % 1) * size[0]) | 0
	var y = ((coord[1] % 1) * size[1]) | 0
...
textureRgbaUnsignedByte.include = ['textureSize'];
textureRgbaUnsignedByte.type = function (node) {
	var samplerType = this.process(node.children[1]).type;
	return this.types[samplerType].type;
};
...
stdlib.shadow2D =
stdlib.shadow3D =
stdlib.texture = textureRFloat32;
stdlib.textureRFloat32 = textureRFloat32;
stdlib.textureRgbaUnsignedByte = textureRgbaUnsignedByte;
stdlib.textureSize = textureSize;
stdlib.textureOffset = textureOffset;

stdlib.setTextureConfig = function(value) {
  if (value === 'RgbaUnsignedByte') {
		stdlib.texture = textureRgbaUnsignedByte
	}
}
