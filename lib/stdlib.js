...
function trunc(x) {
  if (x.length) return x.map(Math.trunc)
  return Math.trunc(x)
}
trunc.type = genType

function round(x) {
	if (x.length) return x.map(Math.round)
	return Math.round(x)
}
round.type = genType
...
/** Текстура представляет собой массив float32 и в шейдере нужно получать float32 в R-канале */
function textureRFloat32(sampler, coord, bias) {
  ...
  if (sampler.data) {
    return sampler.data.slice(idx, idx + 1)
  }
  return sampler.slice(idx, idx + 4)
};

/** Текстура представляет собой массив unsigned byte и в шейдере нужно получать каждый байт в отдельном канале,
 * на каждый тексель приходится по 4 байта */

function textureRgbaUnsignedByteNearest(sampler, coord, bias) {
	var size = textureSize(sampler)
	var x = ((coord[0] % 1) * size[0]) | 0
	var y = ((coord[1] % 1) * size[1]) | 0
	var idx = 4 * (y * size[0] + x)
	if (x < 0 || y < 0 || x >= size[0] || y >= size[1]) {
		return [0]
	}
	/* values имеет тип Uint8Array, если приходит другой тип - это ошибка */
	let values = sampler.data ? sampler.data : sampler
	values = values.slice(idx, idx + 4)
	values = [...values].map(x => x / 255)
	return values
};

function textureRgbaUnsignedByteLinear(sampler, coord, bias) {
	function getRgbaFromTexture(values, size, x, y) {
		const idx = 4 * (y * size[0] + x)
		values = values.slice(idx, idx + 4)
		return values
	}

	const size = textureSize(sampler)
	// Почему нужно вычитать 0.5 описано в "OpenGL ES Version 3.0.6" в "3.8.10.2 Coordinate Wrapping and Texel Selection"
	let x1 = coord[0] * size[0] - 0.5
	let y1 = coord[1] * size[1] - 0.5
	let [dx, dy] = [x1 - Math.trunc(x1), y1 - Math.trunc(y1)]
	x1 = Math.trunc(x1)
  y1 = Math.trunc(y1)
	if (x1 < 0) {
		x1 = 0
	...
	const x2 = x1 + 1 < size[0] ? x1 + 1 : x1
	const y2 = y1 + 1 < size[1] ? y1 + 1 : y1

  if (x1 < 0 || y1 < 0 || x1 >= size[0] || y1 >= size[1]) {
		return [0, 0, 0, 0]
	}

	let values = sampler.data ? sampler.data : sampler
	const a = getRgbaFromTexture(values, size, x1, y1)
	const b = getRgbaFromTexture(values, size, x2, y1)
	const c = getRgbaFromTexture(values, size, x1, y2)
	const d = getRgbaFromTexture(values, size, x2, y2)
	values = [0, 0, 0, 0]
	for (let i = 0; i < 4; i++) {
		values[i] = (a[i] * (1 - dx) + b[i] * dx) * (1 - dy) + (c[i] * (1 - dx)  + d[i] * dx) * dy
	}
	values = [...values].map(x => x / 255)
	return values
}
...
function texture(sampler, coord, bias) {
  const formatType = sampler.formatType || 'RFLOAT32'

	// Нужно сделать выбор между MIN и MAG фильтрами. Пока берем из MIN.
	const textureMinFilter = sampler.textureMinFilter || 'NEAREST'

	if (formatType === 'RFLOAT32') {
		return textureRFloat32(sampler, coord, bias)
	}

	if (textureMinFilter === 'LINEAR') {
		return textureRgbaUnsignedByteLinear(sampler, coord, bias)
	}

	return textureRgbaUnsignedByteNearest(sampler, coord, bias)
}

texture.include = ['textureSize', 'textureRFloat32', 'textureRgbaUnsignedByteNearest', 'textureRgbaUnsignedByteLinear'];
...
function textureOffset(sampler, coord, offset, bias) {
  var size = textureSize(sampler)
  var x = (((coord[0] % 1) * size[0]) | 0) + (offset[0] | 0)
  var y = (((coord[1] % 1) * size[1]) | 0) + (offset[1] | 0)
  var z = coord[2] | 0
  var idx = z * size[0] * size[1] + y * size[0] + x
  if (x < 0 || y < 0 || x >= size[0] || y >= size[1]) {
    return [0]
  }
  if (sampler.data) {
...
textureOffset.include = ['textureSize'];
textureOffset.type = function (node) {
	var samplerType = this.process(node.children[1]).type;
	return this.types[samplerType].type;
};
...
stdlib.textureRFloat32 = textureRFloat32;
stdlib.textureRgbaUnsignedByteNearest = textureRgbaUnsignedByteNearest;
stdlib.textureRgbaUnsignedByteLinear = textureRgbaUnsignedByteLinear;
stdlib.textureSize = textureSize;
stdlib.textureOffset = textureOffset;
