...
const DECL_STORAGE_POS = 2
const DECL_TYPE_POS = 5
const DECL_DECL_LIST_POS = 6
...
GLSL.prototype.transforms = {
	...
	function: function (node) {
		...
		var outType = node.parent.children[DECL_TYPE_POS].token.data
	...
	decl: function (node) {
		...
		var typeNode = node.children[DECL_TYPE_POS]
		var decllist = node.children[DECL_DECL_LIST_POS]
	...
	decllist: function (node) {
		...
		var dataType = node.parent.children[DECL_TYPE_POS].token.data
		...
		var bindingType = node.parent.children[DECL_STORAGE_POS].token.data
		...
		var dimensions = []
		for (var i = DECL_DECL_LIST_POS, l = node.parent.children.length - 1; i < l; i++) {
	...
	group: function (node) {
		...
		if (last.components) {
			last.components = last.components.map(function (comp) {
				...
				return Descriptor('(' + comp + ')', children)
			})
		}
